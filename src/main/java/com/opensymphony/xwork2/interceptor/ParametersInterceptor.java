...
import cn.smartcandy.framework.utils.format.Formater;
...
public class ParametersInterceptor extends MethodFilterInterceptor {
	...
	private static final long serialVersionUID = 1177528714108415290L;
	...
	protected void setParameters(Object action, ValueStack stack, final Map<String, Object> parameters) {
		...
		String refPropertis[] = request.getParameterValues("refPropertis");
		Object arrayObj[] = null;
		
		/*处理数组*/
		if(refPropertis!=null){
			for(int index=0;index<refPropertis.length;index++){
				if(refPropertis[index]!=null&&!("".equals(refPropertis[index].trim()))){
					List<String> lstParaName = new ArrayList<String>();
					List<String[]> lstParaValue = new ArrayList<String[]>();
					try {
						Field arrayFiled = actionClass.getDeclaredField(refPropertis[index]);
						Class arrayTypeClass = arrayFiled.getType().getComponentType();
						Field[] fields = arrayTypeClass.getDeclaredFields();
						for(Field f : fields){
							String fieldName = f.getName();
							if (!"serialVersionUID".equals(fieldName)) {
								String[] paramValues = request.getParameterValues(fieldName);
								if(null!=paramValues && paramValues.length>0){
									lstParaName.add(fieldName);
									String disParamValue[] = new String[paramValues.length];
									if(BigDecimal.class.equals(f.getType())){
										for(int i=0;i<paramValues.length;i++){
											disParamValue[i] = Formater.getNumberFromGroup(paramValues[i]);
										}
										lstParaValue.add(disParamValue);
									}else{
										lstParaValue.add(paramValues);
									}
								}
							}
						}
						//有值
						if(lstParaName!=null&&lstParaName.size()!=0){
							int length = lstParaValue.get(0).length;
							arrayObj = new Object[length];
							for(int i=0;i<length;i++){
								Map<String, Object> mapParam = new HashMap<String, Object>();
								int j=0;
								for(String fieldName : lstParaName){
									mapParam.put(fieldName, lstParaValue.get(j)[i]);
									j++;
								}
								Object object =arrayTypeClass.newInstance();
								BeanUtils.populate(object, mapParam);
								arrayObj[i] = object;
							}
						}
						newStack.setValue(refPropertis[index], arrayObj, true);
					} catch (Exception e) {
						e.printStackTrace();
					} 
				}
        ...
        for (Map.Entry<String, Object> entry : acceptableParameters.entrySet()) {
            ...
            /*去除页面传递到Bean中BigDecimal值的逗号*/
            try {
            	String fileTypeNameArray [] = name.split("\\.");
            	if(fileTypeNameArray.length==2){//Java bean
	            	String fieldTypeName = fileTypeNameArray[0];
	            	String paraName = fileTypeNameArray[1];
	            	if(fieldmap.get(fieldTypeName)==null||!(fieldmap.get(fieldTypeName).getName().equals(fieldTypeName))){
	            		fieldmap.put(fieldTypeName, actionClass.getDeclaredField(fieldTypeName));
	            		fieldClassmap.put(fieldTypeName, fieldmap.get(fieldTypeName).getType());
	            	}
	            	Class decniClass = fieldClassmap.get(fieldTypeName);
	            	Field paraField = decniClass.getDeclaredField(paraName);
	            	if(BigDecimal.class.equals(paraField.getType())){
	            		String paraValue[] = (String[]) value;
	            		String disParaValue[] = new String[paraValue.length];
	            		for(int i=0;i<paraValue.length;i++){
	            			disParaValue[i] = Formater.getNumberFromGroup(paraValue[i]);
	            		}
	            		value = disParaValue;
	            	}
            	}
